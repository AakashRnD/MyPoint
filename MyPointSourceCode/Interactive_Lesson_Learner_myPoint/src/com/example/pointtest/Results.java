...
class DrawView extends View{
	...
	int correct = com.example.pointtest.MainActivity.correct;
	int wrong = com.example.pointtest.MainActivity.wrong;
	int unanswered = com.example.pointtest.MainActivity.unanswered;
	float canvas_width;
	float canvas_height;
	...
	//Drawing pie-graph	
		...
		canvas.drawArc(rectf, 0,correct*360/com.example.pointtest.LaunchScreen.number_questions , true, paint);
		
		paint.setColor(Color.RED);
		canvas.drawRect(260, canvas.getHeight()/2-20, 280, canvas.getHeight()/2, paint);
		canvas.drawArc(rectf, correct*360/com.example.pointtest.LaunchScreen.number_questions,wrong*360/com.example.pointtest.LaunchScreen.number_questions , true, paint);
		
		paint.setColor(Color.BLUE);
		canvas.drawRect(260, canvas.getHeight()/2+20, 280, canvas.getHeight()/2+40, paint);
		canvas.drawArc(rectf,correct*360/com.example.pointtest.LaunchScreen.number_questions + wrong*360/com.example.pointtest.LaunchScreen.number_questions, 360-correct*360/com.example.pointtest.LaunchScreen.number_questions-wrong*360/com.example.pointtest.LaunchScreen.number_questions ,  true, paint);
...
public class Results extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		com.example.pointtest.MainActivity.unanswered=0;
		com.example.pointtest.MainActivity.wrong=0;
		com.example.pointtest.MainActivity.correct=0;
		
		for(int i=0 ;i<com.example.pointtest.LaunchScreen.number_questions ;i++){
			switch (com.example.pointtest.MainActivity.answerState[i]){
			case 0:
				com.example.pointtest.MainActivity.unanswered++;
		break;
			case	 1:
		com.example.pointtest.MainActivity.correct++;
			break;
			case -1:
				com.example.pointtest.MainActivity.wrong++;
		break;
			}	
		}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.pointquiz_results, menu);
		return true;
	}
	@Override
